...
namespace Zeroum.DeveloperEvaluation.ABP.Bruna.WebApi.Controllers
{
    [ApiController]
    [Route("Clients")]
    public class ClientesController : AbpControllerBase
    {
      
        private readonly ILogger<ClientesController> _logger;
        private readonly IClientesRepository _repo;
        public ClientesController(ILogger<ClientesController> logger, IClientesRepository repo)
        {
            _logger = logger;
            _repo = repo;
        }
        ...
        {
            var response = _repo.CreateOrEditClientPJAsync(request);
        ...
        {
            var response = _repo.CreateOrEditClientPFAsync(request);
            if (response.Result > 0)
            {
                return Created(string.Empty, new
                {
        ...
        {
            var response = _repo.GetAllClientsPFAsync();
        ...
        {
            var response = _repo.GetAllClientsPJAsync();
        ...
        {
            var response = _repo.GetByIdForEditPJAsync(id);
        ...
        {
            var response = _repo.GetByIdForEditPFAsync(id);
        ...
        {
            var response = _repo.DeleteClientPJAsync(id);
        ...
        {
            var response = _repo.DeleteClientPFAsync(id);
